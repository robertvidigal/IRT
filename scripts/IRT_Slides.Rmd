...
- **Variáveis latentes.** 
  ...
  - *Nem modelos mais complexos de IRT (GRM, GPCM, Nominal, MIRT).*
...
- **Programação em R**
  - Item-level analysis.
  - Modelo 1PL *a.k.a. Modelo de "Rash"*
  - Modelos 2PL & 3PL
  - IIC & ICC
  - DIF
...
- Como observamos as propriedades das unidades sociais? 
  - **A maioria das tentativas tende a utilizar perguntas em um questionário.**
...
.center[
<img src="https://static.historiadomundo.com.br/conteudo/images/o-santo-graal-foi-uma-das-lendas-mais-conhecidas-toda-idade-media-5cb09971d8c16.jpg" width="225" height="175"/>
]
...
- **Constructo**: inobservável e apenas indiretamente mensurável (e.g., respostas à perguntas em um questionário).

--
  - Um **constructo** é um **conceito abstrato** que é especificamente pensado (ou "criado") para explicar um determinado fenômeno. 
  
  - Um **constructo** pode ser um **conceito** unidimensional (e.g., peso), ou um conjunto de **conceitos** relacionados (e.g., traços de personalidade).

---
# Conceitos, constructos, e variáveis latentes

- A pesquisa científica requer definições operacionais que definam como nossos constructos serão **medidos empiricamente**.
...
- Em estatística, **variáveis latentes** (do latim: *lateo* “está escondido”), não são diretamente observadas, mas sim inferidas a partir de variáveis que podemos medir empiricamente.
...
---
# Teoria Clássica de Mensuração
- **O problema fundamental** é que **T** é não observável. Então, a melhor opção para reduzir o erro é mensurar diferentes propriedades do constructo **T**:
...
- IRT é um conjunto de **modelos matemáticos** para analisar e inferir a **relação entre traços latentes** (característica ou atributo não observável) **e suas manifestações** (i.e., resultados em um teste, respostas de survey ou desempenho em exercícios).
...
- IRT pressupõe que o construto latente (e.g., **estresse, interesse, atitudes**) e os itens de uma medida estão organizados em um continuum não-observável (**θ**). 

  - Logo, o principal objetivo é estabelecer **posição** de cada **indivíduo** nesse **continuum** latente (**θ**).

--

- **Qual é a relação entre o valor verdadeiro (**θ**) e o valor observado?** 
  - Temos que levantar **hipóteses** sobre essa relação...
  - Naturalmente, é necessário **operacionalizar** nossas variávels de interesse.
...
---
# IRT: pressupostos
**Os modelos de IRT implicam em três pressupostos:**
...
- Exemplo: Suponha que estamos interessados em medir a **proficiência em matemática**.
...
--
  ...
  - Logo, aplicamos um instrumento (i.e., teste ou prova) com **cinco perguntas** à pessoa **A**. 
...
- Usamos a letra grega **(δ)** para representar a localização de um item e (**δj**) para representar a j*-ésima* localização do item nesse continuum. 
...
p(x) = \frac{e^z}{(1 + e^z)}
$$
em que **p(x)** é a probabilidade do valor de 1 quando o preditor assume o valor de **x**, **e** é uma constante igual a **2,7182** e **z** é uma combinação linear da(s) variável(is) preditora(s).

---
# O "Modelo de Rasch"
Para especificar **z**, Georg Rasch propôs a ideia de deixar **z** ser a distância entre **θ** e (**δj**) como o determinante da probabilidade de resposta (cf. Rasch 1980).
...
Portanto, esse modelo permitiria prever a probabilidade de uma resposta correta (i.e., 1) em função da localização do item e da pessoa.
...
--
.center[
Esse modelo é chamado de **Modelo de Rasch** (Rasch, 1980). 
]

Essa equação diz que a probabilidade de uma resposta de 1 no item **j** é uma função da distância entre uma pessoa localizada em **θ** e o item localizado em **(δ)**.
...
Usando o modelo de **Rasch** para prever o comportamento da resposta, suponha que administramos um item de matemática localizado em **δ**=1 para indivíduos localizados em **θ**=0. 
...
.center[
<img src ="https://i.ibb.co/hXyCpRG/1pl-rash.png" width=600 height=300 />
]
...
---
# 1-Parameter Logistic Model
- Para isso, revisamos o expoente na equação da **IRF**. Esse expoente, 
$$
(\theta – \delta_j)
$$
...
$$
--
- Como **α** está diretamente relacionado à inclinação da linha de regressão logística, uma alteração em **α** leva a uma alteração na inclinação da curva  
  - Ou seja, a inclinação da **IRF** pode ser modificada alterando-se o valor de **α**.
  
---
# 1-Parameter Logistic Model

- Podemos reescrever a equação **IRF** para incorporar explicitamente o **α** Isso produz o modelo logístico de 1 parâmetro (**1-PL**).
...
.center[
<img src ="https://i.ibb.co/hXyCpRG/1pl-rash.png" width=425 height=225 />
]
...
---
# Parâmetros IRT
- De acordo com a teoria de resposta ao item, a **dificuldade** (**δ**) de um item descreve onde o item está localizado ao longo da escala de habilidade. 

  - Por exemplo, um **item fácil** tem maior probabilidade de acerto entre os examinandos com baixa habilidade e um **item difícil** tem maior probabilidade de acerto entre os examinandos com alta habilidade; portanto, a **dificuldade** é um índice de localização. 
...
--
- Enquanto a **discriminação** **α**, isto é, a **capacidade de um item diferenciar** os indivíduos com habilidades diferentes em relação ao item em questão. 
...
.center[Vamos agora incorporar a **informação** de o quão bem um item **diferencia/discrimina** nossos indíviduos com diferentes níveis de habilidade **θ** no nosso **continuum latente** com o modelo de **2-PL**]

---
# 2-parameter logistic model (2PL)

- Ainda usamos a **distância** entre as localizações da pessoa e do item (**θ - δj**), mas agora ponderamos essa distância de acordo com a capacidade de **discriminação** do item. 
...
- Ao levar em conta a **capacidade de discriminação** de um item, estamos relaxando a **restrição** de que os itens devem compartilhar uma **inclinação comum**. 
  ...
  - Quando isso é feito, o modelo **1-PL** se torna o modelo logístico de **dois parâmetros** (2-PL).
...
- À medida que o valor de **αj** aumenta, a inclinação da **IRF** se torna mais íngreme e a capacidade do item de discriminar entre indivíduos aumenta.
...
- Teoricamente, o parâmetro de discriminação pode variar de **-Inf** a **+Inf**. 
  - Entretanto, valores razoavelmente "bons" de **αj** variam de aproximadamente de **0.8** à **2.5** (Finch 2015). 

--

- Um **αj** negativo reflete um item em que os indivíduos com **θ**s mais baixos têm maior probabilidade de obter uma resposta 1 do que os indivíduos com **θ**s mais altos.
  - Dessa forma, um item com um **αj** negativo está se comportando de forma contraintuitiva.
...
- Agora vamos falar de um modelo para lidar com a chance de sucesso em um item: **3-PL** 
...
- No nosso exame de matemática de múltipla-escolha, alguns indivíduos com baixa proficiência poderão selecionar a opção correta simplesmente **adivinhando**.
  - Nesses casos, a **IRF** tem uma assíntota inferior com algum valor diferente de zero e é isso que o modelo de **3-PL** aborda: a assíntota inferior diferente de **zero**

--

- Para desenvolver o modelo de **3-PL**, precisamos nos perguntar: **"Qual deve ser a probabilidade de uma resposta 1
...
de uma resposta de 0, considerando o modelo **2-PL**.
...
- Com o modelo **3-PL**, há três parâmetros que caracterizam o item **j**:
$$
...
- Em suma, a função do **χ** é refletir que alguns indivíduos com **θ**s infinitamente baixos podem obter uma resposta de 1 quando, de acordo com o **2-PL**, não deveriam.
...
- Embora  **χj** seja considerado um parâmetro do item, ele pode na verdade refletir uma **característica da pessoa** (ou seja, outro parâmetro da pessoa) ou, pelo menos, uma interação entre as características da pessoa e do item.  
  ...
  - As características pessoais incluem não apenas o **θ** de uma pessoa, mas também sua **perspicácia nos testes ou tendências de "tomar riscos"**. Esses dois fatores são **variáveis latentes tangenciais**.

--

- **Problemas de estimação no 3PL**: A superfície de log-likelihood de um item com três parâmetros de item exigiria quatro dimensões para representá-la graficamente. 
  - Em alguns casos, a distinção entre essas várias superfícies pode ser problemática (Samejima 1973). 
...
???
Na demonstração empírica a seguir, vamos sempre codificar o grupo de Referência como 0, e o grupo Foca como 1 e vamos trabalhar apenas com DIF com dados de resposta dicotômica.
---
# Funcionamento diferencial do item (DIF)
